...
import type { Session, ProtocolType, EncodingMode } from '../../types';
import { bytesToHex, bytesToAscii, formatTimestamp } from '../../utils/encoding';
...
function PanelCard({ children }: { children: React.ReactNode }) {
  return <div className="neon-card flex flex-col overflow-hidden">{children}</div>;
}
...
function FieldLabel({ seq, label }: { seq?: number; label: string }) {
  return <label className="block mb-1 uppercase font-bold tracking-wider" style={{ color: '#94a3b8', fontSize: 10 }}>
    ...
    <div className="px-3 pb-3 flex flex-col gap-2">
      ...
      {adding && (
        <div className="flex flex-col gap-1.5 p-2 rounded" style={{ background: 'rgba(16,34,34,0.6)', border: '1px solid rgba(19,236,236,0.15)' }}>
          <input value={name} onChange={e => setName(e.target.value)} placeholder="Name..." className="field-control w-full" style={{ height: 26, fontSize: 11 }} />
          <textarea value={data} onChange={e => setData(e.target.value)} placeholder="Data..." rows={2}
...
function SendHistoryPanel({ history, onSelect }: { history: string[]; onSelect: (t: string) => void }) {
  if (history.length === 0) {
    return <div style={{ color: '#1e3a3a', fontSize: 11, fontFamily: 'var(--font-mono)', textAlign: 'center', padding: '10px 12px' }}>No history yet</div>;
  }
  return (
...
// We expose a global event so DataSend can receive shortcut/history selections
...
export default function ConnectionPanel({ session }: Props) {
  ...
  // Save RX to File — uses File System Access API (available in WebView2)
  const handleSaveToFile = async (checked: boolean) => {
    updateReceive(session.id, { saveToFile: checked });
    if (!checked) {
      if (fileWriterRef.current) { await fileWriterRef.current.close(); fileWriterRef.current = null; }
      appendLog(session.id, { timestamp: Date.now(), direction: 'system', data: Array.from(new TextEncoder().encode('Stopped saving to file')) });
      return;
    }
    try {
      ...
      appendLog(session.id, { timestamp: Date.now(), direction: 'system', data: Array.from(new TextEncoder().encode('Saving received data to file...')) });
    } catch {
      updateReceive(session.id, { saveToFile: false });
    }
  };

  // Called by DataLog when new recv data arrives (via a global hook below)
  React.useEffect(() => {
    // Listen for incoming log entries to write to file
    if (!receiveSettings.saveToFile || !fileWriterRef.current) return;
    const lastLog = session.logs[session.logs.length - 1];
    if (!lastLog || lastLog.direction !== 'recv') return;
    const line = `[${formatTimestamp(lastLog.timestamp)}] RECV: ${
      receiveSettings.encoding === 'HEX' ? bytesToHex(lastLog.data) : bytesToAscii(lastLog.data)
    }\n`;
    fileWriterRef.current.write(line).catch(() => {});
  }, [session.logs.length]);

  // Export current log as download (fallback / manual export)
  const exportLog = () => {
    const lines = session.logs.map(e => {
      const ts   = formatTimestamp(e.timestamp);
      const dir  = e.direction.toUpperCase();
      const text = receiveSettings.encoding === 'HEX' ? bytesToHex(e.data) : bytesToAscii(e.data);
      return `[${ts}] ${dir}: ${text}`;
    });
  ...
  const btnBg    = isActive ? 'linear-gradient(to bottom,rgba(248,113,113,0.15),rgba(248,113,113,0.05))' : 'linear-gradient(to bottom,rgba(19,236,236,0.2),rgba(19,236,236,0.05))';
  ...
  return (
    <>
      {/* ── Network Settings ──────────────────────────── */}
      <PanelCard>
        <PanelHeader icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>} label="Network Settings" />
        <div className="p-3 flex flex-col gap-3">
          <div><FieldLabel seq={1} label="Protocol Type" /><FieldSelect value={config.protocol} onChange={v => updateConfig(session.id, { protocol: v as ProtocolType })} options={PROTOCOLS} /></div>
          {showWs  && <div><FieldLabel seq={2} label="WebSocket URL" /><FieldInput value={config.wsUrl} onChange={v => updateConfig(session.id, { wsUrl: v })} placeholder="ws://127.0.0.1:8080" /></div>}
          {isSrv   && (<><div><FieldLabel seq={2} label="Listen Address" /><FieldInput value={config.localHost} onChange={v => updateConfig(session.id, { localHost: v })} placeholder="0.0.0.0" /></div><div><FieldLabel seq={3} label="Listen Port" /><FieldInput value={String(config.localPort)} onChange={v => updateConfig(session.id, { localPort: Number(v) })} type="number" /></div></>)}
          {showRemote && !showWs && (<><div><FieldLabel seq={2} label="Remote IP Address" /><FieldInput value={config.remoteHost} onChange={v => updateConfig(session.id, { remoteHost: v })} placeholder="127.0.0.1" /></div><div><FieldLabel seq={3} label="Remote Port" /><FieldInput value={String(config.remotePort)} onChange={v => updateConfig(session.id, { remotePort: Number(v) })} type="number" /></div></>)}
          {showLocal && !isSrv && <div><FieldLabel seq={4} label="Local Port" /><FieldInput value={String(config.localPort)} onChange={v => updateConfig(session.id, { localPort: Number(v) })} placeholder="0 (auto)" type="number" /></div>}
          ...
          >
            ...
            {isActive ? 'Disconnect' : isBusy ? 'Connecting…' : 'Connect'}
          </button>
        </div>
      </PanelCard>
      ...
      {/* ── Receive Settings ────────────────────────── */}
      <PanelCard>
        <PanelHeader icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="8 17 12 21 16 17"/><line x1="12" y1="12" x2="12" y2="21"/><path d="M20.88 18.09A5 5 0 0018 9h-1.26A8 8 0 103 16.29"/></svg>} label="Receive Settings" />
        <div className="p-3 flex flex-col gap-2">
          <RadioGroup options={['ASCII', 'HEX']} value={receiveSettings.encoding} onChange={v => updateReceive(session.id, { encoding: v as 'ASCII' | 'HEX' })} />
          <div className="flex flex-col gap-1.5">
            <CheckRow checked={receiveSettings.showAsLog} onChange={v => updateReceive(session.id, { showAsLog: v })} label="Show as Log Mode" />
            <CheckRow checked={receiveSettings.autoNewline} onChange={v => updateReceive(session.id, { autoNewline: v })} label="Auto-newline after RX" />
            <CheckRow checked={receiveSettings.saveToFile} onChange={handleSaveToFile} label="Save RX to File..." />
            <CheckRow checked={receiveSettings.pauseReceiving} onChange={v => updateReceive(session.id, { pauseReceiving: v })} label="Pause Receiving" />
          </div>
          <div className="flex items-center justify-between pt-2" style={{ borderTop: '1px solid rgba(19,236,236,0.1)' }}>
            <button style={linkStyle()} onClick={exportLog} title="Export current log to .txt">Export Log</button>
            <button style={linkStyle()} onClick={() => clearLogs(session.id)}>Clear RX</button>
          </div>
        </div>
      </PanelCard>
      ...
      {/* ── Send Settings ───────────────────────────── */}
      <PanelCard>
        <PanelHeader icon={<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>} label="Send Settings" />
        <div className="p-3 flex flex-col gap-2">
          <RadioGroup options={['ASCII', 'HEX']} value={sendSettings.encoding} onChange={v => updateSend(session.id, { encoding: v as 'ASCII' | 'HEX' })} accent />
          <div className="flex flex-col gap-1.5">
            <CheckRow checked={sendSettings.autoParseEscapes} onChange={v => updateSend(session.id, { autoParseEscapes: v })} label="Auto-parse Escapes" accent />
            <CheckRow checked={sendSettings.autoCRLF} onChange={v => updateSend(session.id, { autoCRLF: v })} label="Auto-CRLF for AT cmds" accent />
            <CheckRow checked={sendSettings.autoChecksum} onChange={v => updateSend(session.id, { autoChecksum: v })} label="Auto-append Checksum" accent />
            {sendSettings.autoChecksum && (
              <FieldSelect value={sendSettings.checksumType} onChange={v => updateSend(session.id, { checksumType: v as 'CRC16' | 'LRC' | 'SUM8' })}
                options={[{ value: 'CRC16', label: 'CRC16 Modbus' }, { value: 'LRC', label: 'LRC' }, { value: 'SUM8', label: 'Checksum8' }]} />
            )}
            <div className="flex items-center gap-2 mt-1 p-1.5 rounded" style={{ background: 'rgba(16,34,34,0.3)', border: '1px solid rgba(19,236,236,0.05)' }}>
              ...
              <span style={{ fontSize: 11, color: '#cbd5e1' }}>Periodic</span>
          ...
          {/* Bottom links */}
          <div className="flex items-center justify-between pt-2" style={{ borderTop: '1px solid rgba(19,236,236,0.1)' }}>
            <button style={linkStyle('var(--color-accent)')} onClick={() => { setShowShortcuts(v => !v); setShowHistory(false); }}>Quick Shortcuts</button>
            <button style={linkStyle('var(--color-accent)')} onClick={() => { setShowHistory(v => !v); setShowShortcuts(false); }}>Send History</button>
          </div>
        </div>
